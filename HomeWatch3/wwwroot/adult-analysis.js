...
function render(){
  ...
  $('unassignedSignals').innerHTML=unassigned.length?unassigned.map(signal=>`<div class="unassigned-item"><strong>${esc(signal.domain)}</strong><span>${signal.signals} signal${signal.signals===1?'':'s'} · ${esc(bytes(signal.bytesDown))} down · ${Number(signal.confidence||0)}% confidence</span><span>${esc(when(signal.firstSeenUtc))} to ${esc(when(signal.lastSeenUtc))}${signal.advertising?' · advertising filter':''}</span></div>`).join(''):'<div class="empty">No unassigned adult signals in this window.</div>';
}
