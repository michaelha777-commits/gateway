...
function row(item){
  ...
  const infra=item.infrastructure?'<span class="badge">INFRASTRUCTURE</span>':'<span class="badge live-badge">USER DEVICE</span>';
  return `<div class="row device-manage-row"><div class="device-main"><div><a class="device-link primary" href="/device.html?id=${d.id}">${escapeHtml(d.name||d.lastIpAddress||'Unknown device')}</a><div class="secondary">${escapeHtml(d.vendor||'Unknown vendor')} • ${escapeHtml(d.lastIpAddress||'No IP')}</div><div class="bandwidth-line">${state}${infra}<span><b>↓ ${escapeHtml(down)}</b></span><span><b>↑ ${escapeHtml(up)}</b></span><span>Total ${escapeHtml(total)}</span></div></div><div class="middle"><div class="secondary">MAC ${escapeHtml(d.macAddress||'Unknown')}</div><div class="secondary">Window ↓ ${escapeHtml(cin)} • ↑ ${escapeHtml(cout)} • ${remoteCount} remote${remoteCount===1?'':'s'}${stamp(t)?` • ${escapeHtml(stamp(t))}`:''}</div></div></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="button small secondary-button" data-id="${d.id}" data-infrastructure="${item.infrastructure?'false':'true'}" type="button">${item.infrastructure?'Mark user device':'Mark infrastructure'}</button><button class="button small ${item.ignored?'secondary-button':'danger-button'}" data-id="${d.id}" data-action="${item.ignored?'remove':'ignore'}" type="button">${item.ignored?'Remove ignore':'Ignore'}</button></div></div>`
}
function renderTopTalkers(){
  const byIp=trafficByIp();
  const monitored=management.filter(x=>!x.ignored&&!x.infrastructure).map(x=>x.device);
...
async function setInfrastructure(id,value,button){button.disabled=true;try{await json(`/api/devices/${id}/infrastructure`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({infrastructure:value})});await load()}catch(e){alert(e.message)}finally{button.disabled=false}}
document.addEventListener('click',e=>{const i=e.target.closest('[data-id][data-infrastructure]');if(i){setInfrastructure(i.dataset.id,i.dataset.infrastructure==='true',i);return}const b=e.target.closest('[data-id][data-action]');if(b)setIgnored(b.dataset.id,b.dataset.action==='ignore',b)});$('deviceSearch').addEventListener('input',render);load();setInterval(load,5000);
