...
let livePaused=false,liveRows=[],management=[];
...
async function setIgnored(id,ignored){
  try{
    await json(`/api/devices/${id}/ignored`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({ignored})});
    await loadDevices();
  }catch(e){alert(`Unable to update device: ${e.message}`)}
}
function render(){
  const ignoredIds=new Set(management.filter(x=>x.ignored).map(x=>Number(x.device.id)));
  const devices=management.map(x=>x.device),byIp=new Map(devices.filter(d=>d.lastIpAddress).map(d=>[d.lastIpAddress,d]));
  const q=($('liveFilter').value||'').trim().toLowerCase();
  const rows=liveRows.filter(r=>{const ip=pick(r,'client','client_ip','source','src','ip');const device=byIp.get(ip);if(device&&ignoredIds.has(Number(device.id)))return false;const domain=pick(r,'domain','name','qname','query');const hay=`${device?.name||''} ${ip} ${domain}`.toLowerCase();return !q||hay.includes(q)}).slice(0,150);
  $('liveActivity').innerHTML=rows.length?rows.map(r=>{const ip=pick(r,'client','client_ip','source','src','ip')||'Unknown IP';const domain=pick(r,'domain','name','qname','query')||'Unknown domain';const type=pick(r,'type','qtype','query_type')||'DNS';const result=pick(r,'return_code','rcode','status','answer')||'';const device=byIp.get(ip);const name=device?.name||ip;const ignoreButton=device?`<button class="button small danger-button" type="button" data-ignore-id="${device.id}">Ignore device</button>`:'';return `<div class="live-row"><div class="live-dot"></div><div class="live-main"><div class="primary">${escapeHtml(domain)}</div><div class="secondary">${escapeHtml(name)} • ${escapeHtml(ip)}</div></div><div class="live-meta"><span class="badge">${escapeHtml(type)}</span>${result?`<span class="secondary">${escapeHtml(result)}</span>`:''}<time>${escapeHtml(age(liveTime(r)))}</time>${ignoreButton}</div></div>`}).join(''):'<div class="empty">No matching live activity from monitored devices.</div>';
