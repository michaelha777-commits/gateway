...
function groupSessions(rows) {
  ...
  for (const row of rows) {
    ...
    if (!group) {
      group = {deviceId:row.deviceId, device:row.device, ip:row.ip, service, rows:[], exactUrls:new Set(), domains:new Set(), applications:new Set(), protocols:new Set(), sources:new Set(), destinations:new Set(), first:new Date(row.startedUtc || row.timestampUtc).getTime(), last:new Date(row.lastSeenUtc || row.timestampUtc).getTime(), blocked:false, background:true, encrypted:false, confidence:0, visibility:'ip', bytesDown:0, bytesUp:0};
      groups.set(key, group);
    }
    group.rows.push(row);
    if (row.exactUrl) group.exactUrls.add(row.exactUrl);
...
function render() {
  ...
  const sessions = groupSessions(visibleRows).filter(session => {
    const haystack = `${session.device || ''} ${session.ip || ''} ${session.service} ${[...session.exactUrls].join(' ')} ${[...session.domains].join(' ')} ${[...session.applications].join(' ')} ${[...session.destinations].join(' ')}`.toLowerCase();
  ...
  $('liveActivity').innerHTML = sessions.length ? sessions.map(session => {
    ...
    const exactUrls = [...session.exactUrls];
    ...
    const primaryTarget = exactUrls[0] || domains[0] || applications[0] || destinations[0] || session.service;
    const targetKind = exactUrls.length ? 'Exact URL' : domains.length ? 'Hostname / SNI' : applications.length ? 'Application' : 'Remote IP';
    const relatedTargetCount = exactUrls.length ? exactUrls.length - 1 : domains.length ? domains.length - 1 : 0;
    const targetClass = exactUrls.length ? 'exact-url-target' : domains.length ? 'hostname-target' : '';
    const detectedApplication = applications[0] || session.service;
    const application = detectedApplication !== primaryTarget ? detectedApplication : '';
    return `<article class="live-row session-row"><div class="live-dot"></div><div class="live-main"><div class="session-target-label">${escapeHtml(targetKind)}</div><div class="session-target ${targetClass}" title="${escapeHtml(primaryTarget)}">${escapeHtml(primaryTarget)}${relatedTargetCount > 0 ? `<span class="session-target-more">+${relatedTargetCount} more</span>` : ''}</div><div class="session-context">${application ? `<span class="session-application">${escapeHtml(application)}</span> • ` : ''}${deviceLink}${session.ip ? ` • ${escapeHtml(session.ip)}` : ''} • ${escapeHtml(sessionDuration(session))}</div><div class="secondary">Last seen ${escapeHtml(age(new Date(session.last).toISOString()))}</div>${trafficSummary(traffic) ? `<div class="secondary wrap-text">${escapeHtml(trafficSummary(traffic))}</div>` : ''}<details class="session-evidence"><summary>Correlated evidence (${session.rows.length} activit${session.rows.length === 1 ? 'y' : 'ies'})</summary><div class="live-evidence-grid"><span><b>Exact URLs:</b> ${escapeHtml(exactUrls.join(', ') || 'Not observed')}</span><span><b>Hostnames:</b> ${escapeHtml(domains.join(', ') || 'Not observed')}</span><span><b>Applications:</b> ${escapeHtml(applications.join(', ') || 'Not identified')}</span><span><b>Protocols:</b> ${escapeHtml(protocols.join(', ') || 'Unknown')}</span><span><b>Sources:</b> ${escapeHtml(sources.join(' + ') || 'Unknown')}</span><span><b>Traffic:</b> ↓ ${escapeHtml(bytes(session.bytesDown))} / ↑ ${escapeHtml(bytes(session.bytesUp))}</span></div></details></div><div class="live-meta"><span class="${badge}">${label}</span><span class="badge visibility-${escapeHtml(session.visibility)}">${escapeHtml(visibilityLabel(session.visibility))}</span>${session.encrypted ? '<span class="badge encrypted-badge">Encrypted</span>' : ''}${totalBytes ? `<span class="badge">${escapeHtml(bytes(totalBytes))}</span>` : ''}<span class="badge">${session.confidence}% confidence</span>${session.deviceId ? `<button class="button small secondary-button" type="button" data-session-device-id="${session.deviceId}">Details</button><details class="manage-menu"><summary>Manage</summary><div class="manage-panel"><button class="button small secondary-button" type="button" data-ignore-id="${session.deviceId}">Hide all activity from this device</button><div class="secondary">This only hides activity. It does not block the device.</div></div></details>` : ''}</div></article>`;
