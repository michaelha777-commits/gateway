...
public sealed record VideoSessionRecord(
    ...
    long SampledAttributedBytesDown = 0,
    long SampledAttributedBytesUp = 0);
    ...
    ILogger<TrafficSessionMonitor> logger) : BackgroundService
{
        ...
        try
        {
            ...
            lock (_gate)
            {
                foreach (var tr in ExtractTraffic(traffic))
                {
                    ...
                    var sampledDown = active.SampledAttributedBytesDown + (long)(bin / 8.0 * elapsed * share);
                    var sampledUp = active.SampledAttributedBytesUp + (long)(bout / 8.0 * elapsed * share);
                    var flowDown = SumFlowBytes(active.FlowEvidence, x => x.BytesDown);
                    var flowUp = SumFlowBytes(active.FlowEvidence, x => x.BytesUp);
                    ...
                    ReplaceSession(active with
                    {
                        ...
                        SampledAttributedBytesDown = sampledDown,
                        SampledAttributedBytesUp = sampledUp,
                        AttributedBytesDown = Math.Max(sampledDown, flowDown),
                        AttributedBytesUp = Math.Max(sampledUp, flowUp),
                        AttributionConfidence = confidence,
                        MatchedRemoteIps = matchedIps
                    });
                }
            ...
            foreach (var row in ExtractDns(dns).OrderBy(ParseDnsTime))
            {
                ...
                if (string.IsNullOrWhiteSpace(domain)
                    || ignoredDomains.IsIgnored(domain)
                    || domainPolicy.IsTrusted(domain)
                    || string.IsNullOrWhiteSpace(ip)
                    || !byIp.TryGetValue(ip, out var device)) continue;
    ...
    private void ProcessNtopngFlows(NtopngFlowSnapshot snapshot, IReadOnlyDictionary<string, Device> byIp)
    {
        ...
        foreach (var flow in snapshot.Flows)
        {
            ...
            if (!string.IsNullOrWhiteSpace(hostname)
                && (ignoredDomains.IsIgnored(hostname) || domainPolicy.IsTrusted(hostname))) continue;
            ...
            lock (_gate)
            {
                ...
                var flowEvidence = existing!.FlowEvidence
                    ...
                    .TakeLast(500)
                    .ToArray();
                var flowBytesDown = SumFlowBytes(flowEvidence, x => x.BytesDown);
                var flowBytesUp = SumFlowBytes(flowEvidence, x => x.BytesUp);
                ...
                ReplaceSession(existing with
                {
                    ...
                    AttributedBytesDown = Math.Max(existing.SampledAttributedBytesDown, flowBytesDown),
                    AttributedBytesUp = Math.Max(existing.SampledAttributedBytesUp, flowBytesUp),
    ...
    private static VideoSessionRecord MergePair(VideoSessionRecord a, VideoSessionRecord b)
    {
        ...
        var evidence = a.Evidence.Concat(b.Evidence)
            .GroupBy(x => new { x.TimestampUtc, x.Domain, x.QueryType, x.Action })
            .Select(g => g.First())
            .OrderBy(x => x.TimestampUtc)
            .TakeLast(500)
            .ToArray();
        var flowEvidence = a.FlowEvidence.Concat(b.FlowEvidence)
            .GroupBy(x => x.FlowId, StringComparer.Ordinal)
            .Select(g => g.OrderByDescending(x => x.LastSeenUtc).First())
            .OrderBy(x => x.StartedUtc)
            .TakeLast(500)
            .ToArray();
        var overlaps = TimeWindowsOverlap(a, b);
        var sampledDown = MergeSampledCounter(a.SampledAttributedBytesDown, b.SampledAttributedBytesDown, overlaps);
        var sampledUp = MergeSampledCounter(a.SampledAttributedBytesUp, b.SampledAttributedBytesUp, overlaps);
        var flowDown = SumFlowBytes(flowEvidence, x => x.BytesDown);
        var flowUp = SumFlowBytes(flowEvidence, x => x.BytesUp);
        return a with
        {
            ...
            BytesDown = MergeSampledCounter(a.BytesDown, b.BytesDown, overlaps),
            BytesUp = MergeSampledCounter(a.BytesUp, b.BytesUp, overlaps),
            ...
            BlockedRequests = evidence.Count(x => x.Action.Equals("Block", StringComparison.OrdinalIgnoreCase)),
            Active = active,
            SampledAttributedBytesDown = sampledDown,
            SampledAttributedBytesUp = sampledUp,
            AttributedBytesDown = Math.Max(sampledDown, flowDown),
    ...
    private static bool TimeWindowsOverlap(VideoSessionRecord a, VideoSessionRecord b) =>
        a.StartedUtc <= b.LastSeenUtc && b.StartedUtc <= a.LastSeenUtc;

    private static long MergeSampledCounter(long a, long b, bool overlaps) =>
        overlaps ? Math.Max(Math.Max(0, a), Math.Max(0, b)) : Math.Max(0, a) + Math.Max(0, b);

    private static long SumFlowBytes(IEnumerable<SessionFlowEvidence> flows, Func<SessionFlowEvidence, long> selector) =>
    ...
    private static VideoSessionRecord NormalizeAccounting(VideoSessionRecord session)
    {
        var flowDown = SumFlowBytes(session.FlowEvidence, x => x.BytesDown);
        var flowUp = SumFlowBytes(session.FlowEvidence, x => x.BytesUp);
        var sampledDown = session.SampledAttributedBytesDown;
        var sampledUp = session.SampledAttributedBytesUp;

        // Alpha.30 persisted only the combined attributed counters. If exact
        // per-flow evidence exists, discard that legacy combined value because it
        // may already contain the same bytes from a coalesced DNS/flow fragment.
        if (session.FlowEvidence.Length == 0)
        {
            sampledDown = Math.Max(sampledDown, session.AttributedBytesDown);
            sampledUp = Math.Max(sampledUp, session.AttributedBytesUp);
        }
    ...
    private void Load()
    {
        try
        {
            ...
            lock (_gate)
            {
                _sessions.AddRange(data.Select(x => NormalizeAccounting(x with
                {
