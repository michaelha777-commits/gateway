        ...
        try
        {
            ...
            lock (_gate)
            {
                foreach (var tr in ExtractTraffic(traffic))
                {
                    ...
                    if (active is not null) ReplaceSession(active with { BytesDown = active.BytesDown + (long)(bin / 8.0 * elapsed), BytesUp = active.BytesUp + (long)(bout / 8.0 * elapsed), PeakBitsIn = Math.Max(active.PeakBitsIn, bin), PeakBitsOut = Math.Max(active.PeakBitsOut, bout) });
                }
            ...
            foreach (var row in ExtractDns(dns).OrderBy(ParseDnsTime))
            {
                ...
                var domain = GetString(row, "domain", "name", "qname", "query")?.ToLowerInvariant(); var ip = GetString(row, "client", "client_ip", "src", "ip");
                ...
                if (service.Value.Adult && !adultClassifier.Classify(domain).IsAdult) continue;
                ...
                lock (_gate)
                {
                    ...
                    else ReplaceSession(active with { LastSeenUtc = when > active.LastSeenUtc ? when : active.LastSeenUtc, Domains = active.Domains.Append(domain).Distinct(StringComparer.OrdinalIgnoreCase).Take(100).ToArray(), Policies = string.IsNullOrWhiteSpace(policy) ? active.Policies : active.Policies.Append(policy).Distinct(StringComparer.OrdinalIgnoreCase).ToArray(), BlockedRequests = active.BlockedRequests + (action.Equals("Block", StringComparison.OrdinalIgnoreCase) ? 1 : 0) });
                }
            }
