...
public static class V2Enhancements
{
    ...
    {
        services.AddHostedService(sp => (AdultDomainClassifier)sp.GetRequiredService<IAdultDomainClassifier>());
    ...
    public static void MapV2Enhancements(this WebApplication app)
    {
        ...
        app.MapPost("/api/intelligence/adult/safe", (SafeDomainUpdate u, IAdultDomainClassifier c) =>
        {
            ...
            return d is null?Results.BadRequest(new{error="Enter a valid domain."}):Results.Ok(new{domain=d});
        });
        app.MapDelete("/api/intelligence/adult/safe", (string domain,IAdultDomainClassifier c) => {((AdultDomainClassifier)c).RemoveSafeDomain(domain);return Results.Ok(new{domain});});
            ...
            int minutes=60,long? deviceId=null,string? category=null,string? search=null,int limit=200,CancellationToken ct=default) =>
        {
            ...
            var filtered=rows.Where(x=>string.IsNullOrWhiteSpace(x.Domain)||!ignoredDomains.IsIgnored(x.Domain)).Select(x=>new{
                x.Id,x.TimestampUtc,x.DeviceId,device=x.DeviceId.HasValue&&devices.TryGetValue(x.DeviceId.Value,out var d)?d.Name??d.LastIpAddress:x.SourceIp,
                ip=x.SourceIp,x.Domain,x.Category,x.Protocol,x.Source,x.Confidence,x.Blocked
            }).ToArray();
