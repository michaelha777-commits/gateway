const HW1_VERSION = '2.0.0-alpha.11';
...
function hw1Category(domain, storedCategory) {
  ...
  const rules = [
    ...
    [/doubleclick|googlesyndication|adservice|adsystem|taboola|outbrain|tracking|analytics/, 'advertising'],
...
function hw1Description(domain, category) {
  ...
  const descriptions = {
    ...
    streaming: 'Video or audio streaming service',
    social: 'Social-media or community service',
    advertising: 'Advertising, marketing, or tracking infrastructure',
    development: 'Software-development or cloud service',
    productivity: 'Work, communication, or productivity service',
    shopping: 'Retail or online-shopping service',
    gaming: 'Gaming platform or game infrastructure',
    'encrypted-dns': 'Encrypted DNS or alternate name-resolution service',
    bypass: 'VPN, proxy, Tor, or possible filtering-bypass service',
    security: 'Potential security-risk domain',
...
async function loadHw1Alerts() {
  ...
  try {
    ...
    list.innerHTML = alerts.length ? alerts.map(a => `<article class="panel" style="padding:18px;margin-bottom:12px;opacity:${a.acknowledged ? '.62' : '1'}"><div class="panel-heading"><div><span class="eyebrow">${escapeHtml(String(a.severity || 'medium').toUpperCase())}</span><h2>${escapeHtml(a.title)}</h2></div><time>${escapeHtml(formatFullTime(a.createdAt))}</time></div><p><strong>${escapeHtml(a.deviceName || a.deviceIp || 'Unknown device')}</strong> · ${escapeHtml(a.deviceIp || '')}</p><p>${escapeHtml(a.detail || '')}</p>${a.acknowledged ? `<p class="muted">Acknowledged ${escapeHtml(formatFullTime(a.acknowledgedAt))}</p>` : `<button class="ack-alert" data-alert-id="${escapeHtml(a.id)}">Acknowledge</button>`}</article>`).join('') : '<div class="empty">No matching alerts.</div>';
...
function exportHw1(format) {
  ...
  if (format === 'json') {
    content = JSON.stringify(events, null, 2); type = 'application/json'; extension = 'json';
  } else {
