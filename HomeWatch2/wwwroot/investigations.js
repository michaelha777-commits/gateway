(() => {
  ...
  function build(events) {
    ...
    for (const event of sorted) {
      ...
      if (!item || timestamp - date(item.end) > 15 * 60 * 1000) {
        item = { id: `${deviceKey}-${timestamp.getTime()}`, deviceId:event.deviceId || '', deviceName:event.deviceName || event.deviceIp || 'Unknown device', deviceIp:event.deviceIp || '', start:event.timestamp, end:event.timestamp, events:[] };
        list.push(item);
      }
      item.end = event.timestamp;
      item.events.push({...event, investigationType:typeOf(event)});
    }
    const result = [...byDevice.values()].flat();
  ...
  function card(item) {
    ...
    return `<article class="panel investigation-card" data-investigation-id="${esc(item.id)}">
      ...
      <div class="investigation-main"><div class="investigation-title"><span class="device-avatar">${esc(item.deviceName.slice(0,1).toUpperCase())}</span><div><h3>${esc(label(item.primaryType))}</h3><button type="button" data-open-device="${esc(item.deviceId)}">${esc(item.deviceName)}</button><small>${esc(item.deviceIp)}</small></div></div>
      <p>${esc(summary(item))}</p><div class="investigation-tags">${Object.entries(item.counts).sort((a,b)=>b[1]-a[1]).slice(0,4).map(([key,value])=>`<span>${esc(label(key))}: ${value}</span>`).join('')}</div></div>
      <div class="investigation-stats"><strong>${item.confidence}%</strong><span>confidence</span><strong>${item.events.length}</strong><span>DNS events</span><strong>${item.domains.length}</strong><span>domains</span></div>
  ...
  function summary(item) {
    ...
    if (item.primaryType === 'adult') return `HomeWatch detected ${item.domains.filter(d=>d.type==='adult').length} adult-related domain(s) during this activity window. Supporting network requests included ${top || 'related infrastructure'}.`;
    if (item.primaryType === 'privacy-proxy') return `HomeWatch detected encrypted-DNS, private-relay, VPN, or proxy indicators. Review the timeline to determine whether monitoring may have been bypassed.`;
    return `${item.deviceName} contacted ${item.domains.length} unique domain(s), led by ${top || 'general network infrastructure'}.`;
  }
  ...
  function render() {
    ...
    const filtered = investigations.filter(item => {
      const searchMatch = !search || [item.deviceName,item.deviceIp,...item.domains.map(d=>d.domain)].some(v=>String(v||'').toLowerCase().includes(search));
  ...
  async function load() {
    ...
    $('sessionList').innerHTML = '<div class="panel empty">Building investigations…</div>';
    try {
  ...
  function open(id) {
    ...
    $('investigationDrawerContent').innerHTML = `<div class="investigation-header"><div><span class="eyebrow">INVESTIGATION</span><h2>${esc(label(item.primaryType))}</h2><p>${esc(item.deviceName)} · ${esc(item.deviceIp)}</p></div><div class="score"><strong>${item.confidence}%</strong><span>confidence</span></div></div>
      <section class="investigation-summary"><h3>Assessment</h3><p>${esc(summary(item))}</p><div class="investigation-facts"><span><b>Risk</b>${esc(item.risk)}</span><span><b>Duration</b>${esc(duration(item.durationSeconds))}</span><span><b>Events</b>${item.events.length}</span><span><b>Domains</b>${item.domains.length}</span></div></section>
      ...
      <section><h3>Domain evidence</h3><div class="domain-evidence">${item.domains.map(d=>`<article><div><strong>${esc(d.domain)}</strong><span>${esc(label(d.type))}</span></div><b>${d.count}</b></article>`).join('')}</div></section>
    ...
    $('exportInvestigation').onclick = () => { const blob=new Blob([JSON.stringify(item,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`homewatch-investigation-${item.id}.json`; a.click(); URL.revokeObjectURL(a.href); };
