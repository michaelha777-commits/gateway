...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$new1 = '$EventsPath = Join-Path $DataDir ''events.jsonl''' + [Environment]::NewLine + '$EventDiskCachePath = Join-Path $DataDir ''event-view-cache.clixml''';" ^
  ...
  "        try {" ^
  "            $diskCache = Import-Clixml -Path $EventDiskCachePath" ^
  "            if ([int]$diskCache.hours -eq $Hours -and [int64]$diskCache.eventStamp -eq [int64]$eventStamp -and [int64]$diskCache.configStamp -eq [int64]$configStamp) {" ^
  "                $diskResult = @($diskCache.events)" ^
  "                $script:EventViewCache = @{$cacheKey=$diskResult}" ^
  "                Write-Host ('Event cache: loaded {0} events from disk' -f $diskResult.Count)" ^
  "                return $diskResult" ^
  "            }" ^
  "        } catch {" ^
  "            Write-Warning ('Could not load persistent event cache: ' + $_.Exception.Message)" ^
  "        }" ^
  "    }" ^
  "'@;" ^
