...
if ($core -notmatch 'HOMEWATCH_ONLINE_ADULT_INTELLIGENCE_V132') {
    if ($core -notmatch "'xxxjmp\.com'") {
        $core = Replace-Required $core "    'storageimagedisplay.com','sx.cdn.live'" "    'storageimagedisplay.com','sx.cdn.live','xxxjmp.com','pvvstream.pro','pvvstream.com'" 'adult CDN list'
    }
...
'@
    $core = Replace-Required $core 'function Update-Categories([bool]$Force = $false) {' ($onlineCode + 'function Update-Categories([bool]$Force = $false) {') 'category-update function'
...
'@
    $core = Replace-Required $core '    if (Test-Suffix $d $BypassDomains) {' ($classificationInsert + '    if (Test-Suffix $d $BypassDomains) {') 'domain-classification insertion point'

    $identityOld = "    `$category='Application or web service'; `$basis='General hostname classification'; `$confidence=45"
    $identityNew = "    if (Test-Suffix `$d `$AdultSites -or Test-Suffix `$d `$AdultCdns -or Test-OnlineAdultDomain `$d -or `$d -match '(^|[.-])(xxx[a-z0-9-]*|porn[a-z0-9-]*|hentai[a-z0-9-]*)([.-]|`$)') { return [ordered]@{owner='Adult-content service';category='Adult website or media infrastructure';confidence=92;basis='HomeWatch adult-domain intelligence'} }`r`n    `$category='Application or web service'; `$basis='General hostname classification'; `$confidence=45"
    if (-not $core.Contains($identityOld)) {
        $identityNew = $identityNew -replace "`r`n","`n"
    }
    $core = Replace-Required $core $identityOld $identityNew 'domain identity insertion point'

    $updateOld = '                if ($body.updateNow) { [void](Update-Categories $true); Import-Categories }'
    $updateNew = '                if ($body.updateNow) { [void](Update-Categories $true); Import-Categories; [void](Update-OnlineAdultDomains $true); [void](Import-OnlineAdultDomains); $script:EventViewCache=@{} }'
    $core = Replace-Required $core $updateOld $updateNew 'Update categories now handler'

    $startupOld = 'Write-Host "HomeWatch is running at http://127.0.0.1:$Port" -ForegroundColor Green'
    $startupNew = 'try { [void](Update-OnlineAdultDomains); [void](Import-OnlineAdultDomains) } catch { Write-Warning "Initial online adult-domain load failed: $($_.Exception.Message)" }' + "`r`n" + $startupOld
    if (-not $core.Contains($startupOld)) {
        $startupOld = 'Write-Host "HomeWatch is running at http://127.0.0.1:$Port" -ForegroundColor Green'
    }
    $core = Replace-Required $core $startupOld $startupNew 'HomeWatch startup message'
}
...
if ($app -notmatch 'Adult video delivery \(database\)') {
    ...
    $app = Replace-Required $app $old $new 'event enrichment function in web/app.js'
}
